...
static void
fir_filter_c(short *output, const short* input,
 ...
 */
jstring
Java_com_liangfu_pwptracker_TestActivity_stringFromJNI( JNIEnv* env,
                                                        jobject thiz )
{
    char*  str;
    uint64_t features;
    ...
    /* check the result, just in case */
    {
        ...
        for (nn = 0; nn < FIR_OUTPUT_SIZE; nn++) {
            if (fir_output[nn] != fir_output_expected[nn]) {
                if (++fails < 16)
                    D("neon[%d] = %d expected %d",
                      nn, fir_output[nn], fir_output_expected[nn]);
            }
        }
        D("%d fails\n", fails);
    }
